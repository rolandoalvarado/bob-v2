...
class Html
  ...
  def write_html
    ['index', 'header', 'toc', 'intro'].each do |file_name|
      template = File.open(File.expand_path("../templates/#{ file_name }.html.erb", __FILE__), 'r')
      erb      = ERB.new(template.read)
      File.open(File.join(@report_dir, "#{ file_name }.html"), 'w') { |file| file.write(erb.result(binding)) }
    end

    feature_files_path = File.join(@report_dir, "feature_files")
    Dir.mkdir(feature_files_path) unless Dir.exists?(feature_files_path)
    ...
    @results.keys.each do |category_name|
      @results[category_name].each do |feature|
        ...
        erb      = ERB.new(template.read)
        File.open(File.join(@report_dir, "feature_files/#{ underscored_name }.html"), 'w') { |file| file.write(erb.result(binding)) }
      end
    end
  end
