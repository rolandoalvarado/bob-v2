Feature: Add a member to a project
  ...
  This term combines 'organization' and 'cloud' from the old mCloud.
  ...
  Scenario: Amanda adds Albert to one of her projects
    When she adds Albert to the HRIS project
    Then Albert should be able see the following elements of the project:
      | ELEMENT |
      | VMs     |
      | Volumes |
      | Images  |
  ...
  @stretchgoal
  Scenario: Amanda adds Albert and includes a message
    ...
    Amanda
    """
    And Albert should be able see the following elements of the project:
      | ELEMENT |
      | VMs     |
      | Volumes |
      | Images  |
